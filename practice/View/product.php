                ...
                <div class="carousel-inner">
                    <?php for ($i = 0; $i < count($data['images']); $i++) { ?>
                        <div class="carousel-item <?php if ($i == 0) { ?> active <?php } ?> ">
                            <img class="d-block w-100" src="/View/Image/<?= $data['images'][$i]->getImg(); ?>"
                                 alt="First slide">
                        </div>
                    <?php } ?>
                </div>
        ...
        <div class="row">
            <ul class="nav nav-tabs mt-4" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="delivery-tab" data-toggle="tab" href="#delivery" role="tab"
                       aria-controls="delivery" aria-selected="true">Доставка</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="payment-tab" data-toggle="tab" href="#payment" role="tab"
                       aria-controls="payment" aria-selected="false">Оплата</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="guarantee-tab" data-toggle="tab" href="#guarantee" role="tab"
                       aria-controls="guarantee" aria-selected="false">Гарантия</a>
                </li>
            </ul>
            <div class="tab-content pt-3" id="myTabContent">
                <div class="tab-pane fade show active" id="delivery" role="tabpanel" aria-labelledby="delivery-tab">
                    <ol>
                        <li>
                            Курьером по Киеву. Стоимость доставки оборудования - 50 грн. Обычно, если вы разместили
                            заказ до 12:00 — мы доставим его в тот же день. В любом случае, во время заказа наши
                            менеджеры сразу согласуют с вами время доставки.
                        </li>
                        <li>
                            Доставка с услугой Мультитест по Киеву, Харькову и пригороду. Стоимость услуги - 100 грн,
                            выезд за пределы города оплачивается из расчета 3 грн за км в обе стороны. (4 грн зимой)
                        </li>
                        <li>
                            Доставка по Украине сторонними службами доставки. Мы доставляем товары с помощью <br>служб
                            доставки "Новая почта" и "Укрпочта". Стоимость доставки - 50 грн. Доставка осуществляется
                            только после полной предоплаты товара.
                        </li>
                    </ol>
                </div>
                <div class="tab-pane fade" id="payment" role="tabpanel" aria-labelledby="payment-tab">
                    <ol>
                        <li>
                            Наличная оплата возможна только при доставке в Киеве или заказе услуги Мультитест в Киеве и
                            Харькове. Оплата производится исключительно в национальной валюте. В подтверждение оплаты мы
                            выдаем вам наряд.
                        </li>
                        <li>
                            Безналичная оплата осуществляется следующим способом: после оформления заказа, менеджер
                            электронной почтой вышлет Вам счет-фактуру, который Вы сможете оплатить в кассе отделения
                            любого банка или с расчетного счета Вашей фирмы. Для юридических лиц пакет всех необходимых
                            документов предоставляется вместе с товаром.
                        </li>
                        <li>
                            С помощью системы Интернет-расчетов WebMoney клиенты имеют возможность производить
                            моментальную оплату через Интернет. Чтобы оплатить покупку товара в WMZ, необходимо
                            воспользоваться программой Webmoney Keeper или онлайн-интерфейсом Webmoney Keeper Light.
                        </li>
                    </ol>
                </div>
                <div class="tab-pane fade" id="guarantee" role="tabpanel" aria-labelledby="guarantee-tab">
                    <p style="text-indent: 25px;">
                        На товары предоставляется гарантия, подтверждающая обязательства по отсутствию в товаре
                        заводских дефектов. Гарантия предоставляется на срок от 2-х недель до 36 месяцев в зависимости
                        <br> от сервисной политики производителя. Срок гарантии указан в описании каждого товара на
                        нашем сайте. Подтверждением гарантийных обязательств служит гарантийный талон производителя, или
                        наряд нашей компании.
                    </p>
                    <p style="text-indent: 25px;">
...
<div class="tab-content pt-3" id="myTabContent">
    ...
    <div class="tab-pane fade" id="comment" role="tabpanel" aria-labelledby="comment-tab">
        <div class="container">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <textarea class="form-control" id="text_comment" rows="4"
                              placeholder="Добавьте Ваш комментарий"></textarea>
                    <div class="row d-flex justify-content-end">
                        <div class="col-auto">
                            <a class='btn btn-primary mt-2 text-white'
                               onclick="AjaxAddComment('text_comment', <?= $data['product'][0]->getId(); ?>)"><img
                                        class="mr-2" src='/View/Image/add_comment.png'>Добавить</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="shadow mb-5 bg-white rounded">
                    <div class="card bg-border mb-4">
                        <div class="card-header bg">
                            <div class="row">
                                <div class="col">
                                    <div class="font-comment-name"><?= $data['client'][$i]->getFirstName(); ?></div>
                                    <span class="font-comment-date">Дата добавления: <?= $data['comments'][$i]->getDateAdded(); ?></span>
                                </div>
                                <div class="row d-flex justify-content-end">
                                    <?php
                                    if ($_SESSION['user_id'] == $data['comments'][$i]->getClientId()) {
                                        ?>
                                        <div class="col-auto">
                                            <a onclick="AjaxRemoveComment(<?= $data['comments'][$i]->getId(); ?>)"
                                               class="badge badge-warning btn">Delete own comment</a>
                                        </div>
                                        <?php
                                    }
                                    ?>
                        ...
                        <div class="card-body">
                            <p class="card-text "><?= $data['comments'][$i]->getContent(); ?></p>
                        </div>
                    </div>
                </div>
                <?php
